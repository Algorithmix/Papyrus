...
namespace Algorithmix.Forensics
{
    public class Filter
    {
        ...
        public static Image<TColor, TDepth> Reverse<TColor, TDepth>(
            Image<TColor, TDepth> image)
            where TColor : struct,
                IColor where TDepth : new()
        {
            Bitmap reversed = image.Bitmap;
            reversed.RotateFlip(RotateFlipType.Rotate180FlipNone);
            return new Image<TColor, TDepth>(reversed);
        }
        ...
        {
            Bitmap reversed = new Bitmap(image);
            reversed.RotateFlip(RotateFlipType.Rotate180FlipNone);
            return reversed;
        }
